...
def fetch_page(url: str, session: requests.Session, use_browser: bool) -> BeautifulSoup:
    if use_browser:
...
def render_page_with_playwright(url: str) -> str:
...
def log_js_fallback_hint() -> None:
    logging.info(
        "If IKEA renders products via JS, rerun with --render=playwright or "
        "use Selenium as a fallback for HTML rendering before parsing."
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="IKEA UAE scraper + CLIP ingestion.")
    parser.add_argument(
        "--max-pages",
        type=int,
        default=5,
        help="Maximum number of category pages to crawl.",
    )
    parser.add_argument(
        "--render",
        choices=["requests", "playwright"],
        default="requests",
        help="HTML rendering mode (requests or playwright).",
    )
    parser.add_argument(
        "--debug-html",
        type=str,
        default="",
        help="Optional file path to save HTML when no products are found.",
    )
...
def main() -> int:
    ...
    try:
        for page_number in range(1, args.max_pages + 1):
            ...
            page = fetch_page(page_url, session, args.render == "playwright")
            cards = fetch_product_cards(page)
            if not cards:
                if args.debug_html:
                    with open(args.debug_html, "w", encoding="utf-8") as handle:
